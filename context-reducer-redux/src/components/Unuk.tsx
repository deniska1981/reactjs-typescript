...
const Unuk = () => {
    ...
    return (
        <div style={{backgroundColor: `hsl(${theme} 100% ${colorLightness}%)`}}>    
            ...
            <ul>
            {movies.map((clan,index)=><li key={index}>{clan}</li>)}
            </ul>
            <button onClick={()=>dispatch(izbaciPrviFilm())}>Izbaci prvog</button>
            <button onClick={()=>dispatch(addMovie("dozvola za brak"))}>Dodaj "dozvola za brak"</button>
        </div>
    )
}
